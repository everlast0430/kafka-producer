...
class RealTimeBicycle:
	...
	def call(self):
		# url 형태: http://openapi.seoul.go.kr:80/(인증키)/json/bikeList/1/5
		base_url = f'{self.api_url}/{self.auth_key}/{self.dataset_nm}'
		start = 1
		end = 1000
		total_rows = []
		while True:
			...
			if rslt_code:
				...
				if rslt_code == 'INFO-200':
					return total_rows
				else:
					rslt_msg = contents.get('MESSAGE')
					self.log.error(f'요청 실패, 에러코드: {rslt_code}, 메세지:{rslt_msg}')
					time.sleep(30)
	...
	def _get_logger(self):
		...
		handler = TimedRotatingFileHandler(os.path.join(self.log_dir, 'call_bicycle.log'), when="midnight", backupCount=7)
